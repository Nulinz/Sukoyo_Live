...
<link rel="stylesheet" href="<?php echo e(asset('assets/css/bill.css')); ?>">
    ...
    <div class="bill-div">
        <div class="bill-header">
            ...
            <div class="bill-head-right">
                ...
                <a href="<?php echo e($redirectRoute); ?>">
            ...
            <div class="bill-body-right pt-3">
                    ...
                    <div class="mt-2">
                        ...
                        <div class="d-flex justify-content-between align-items-center">
                            <p class="mb-0 text-muted" id="selectedCustomer">
                                <?php if(!empty($customer_name) && !empty($customer_contact)): ?>
                                    <?php echo e($customer_name); ?> - <?php echo e($customer_contact); ?>
                            ...
                            <div class="d-flex flex-column align-items-end gap-1">
                                <div class="d-flex align-items-center gap-2">
                                    <img src="<?php echo e(asset('assets/images/icon_7.png')); ?>" alt="Points" height="18">
                                    <span class="text-muted" id="loyaltyPointsDisplay">
                                        <?php if(!empty($customer_name) && !empty($customer_contact)): ?>
                                            <?php echo e(\App\Models\Customer::where('contact', $customer_contact)->value('loyalty_points') ?? 0); ?>
                                    ...
                                    <div id="splitPaymentInputs" style="display: none;">
                                        <div class="mb-2">
                                            ...
                                            <input type="number" class="form-control" id="cashAmount"
                                                placeholder="Enter cash amount" value="0" min="0" step="0.01">
                                        </div>
                                        <div>
                                            <label class="form-label small">Online Amount</label>
                                            <input type="number" class="form-control" id="onlineAmount"
                                                placeholder="Enter online amount" value="0" min="0" step="0.01">
                                        </div>
                                    </div>
                                </th>
                ...
                <div class="offerbtns mt-3">
                    <form id="posForm">
                        ...
                        <button type="button" class="listbtn px-1 mb-2" onclick="validateAndOpenCorporateBill()"
                ...
                <form id="discountForm">
                    <div class="row">
                        <div class="col-sm-12 col-md-12 mb-3">
                            ...
                            <div class="d-flex align-items-center gap-3 mt-2">
                                <div class="form-check">
                                    <input class="form-check-input" type="radio" name="discountTiming" value="before"
                                        id="before" checked>
                                    <label class="form-check-label" for="before">Before Tax</label>
                                </div>
                                <div class="form-check">
                                    <input class="form-check-input" type="radio" name="discountTiming" value="after"
                                        id="after">
                            ...
                            <div class="d-flex align-items-center gap-3 mt-2">
                                <div class="form-check">
                                    <input class="form-check-input" type="radio" name="discountType" value="percentage"
                                        id="discountPercentage" checked>
                                    <label class="form-check-label" for="discountPercentage">Percentage (%)</label>
                                </div>
                                <div class="form-check">
                                    <input class="form-check-input" type="radio" name="discountType" value="amount"
                                        id="discountAmount">
                        ...
                        <div class="col-sm-12 col-md-12 mb-3">
                            ...
                            <input type="number" class="form-control" id="percentage" min="0" max="100" step="0.01"
                                placeholder="Enter percentage">
                        </div>

                        <div class="col-sm-12 col-md-12 mb-3">
                            <label for="discamount" class="form-label">Amount (₹)</label>
                            <input type="number" class="form-control" id="discamount" min="0" step="0.01"
...
<?php echo $__env->make('corporate_bill', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
<?php echo $__env->make('pos.pos_popups', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
...
<script>
    ...
    function loadItemsDataChunked() {
        ...
        const storeId = '<?php echo e($store_id ?? ""); ?>';
        const role = '<?php echo e(Session::get("role")); ?>';
    ...
    function processBarcode() {
        ...
        // Find ALL matching items (do NOT filter stock here)
        const matchingItems = itemsData.filter(item => {

            if (item.item_code &&
                item.item_code.toString().toLowerCase() === normalizedBarcode) {
                return true;
            }

            if (item.barcode &&
                item.barcode.toString().toLowerCase() === normalizedBarcode) {
                return true;
            }
        ...
        // ✅ MULTIPLE ITEMS → SHOW MODAL (even if stock = 0)
        ...
        // ❌ Out of stock check
        if (stock <= 0) {
            const itemTypeText = item.item_type === 'batch'
                ? `(Batch: ${item.batch_no})`
                : '';

            showFeedback(
                `✗ Out of stock: ${item.item_name} ${itemTypeText}`,
                'error'
            );
        ...
        if (item.item_type === 'batch' && item.exp_date) {
            ...
            if (expDate < new Date()) {
                showFeedback(
                    `✗ Cannot add expired batch: ${item.item_name} (Batch: ${item.batch_no})`,
                    'error'
                );
                barcodeInput.value = '';
                setTimeout(() => {
        ...
        const itemTypeText = item.item_type === 'batch'
            ? `batch item ${item.batch_no}`
            : 'item';

        showFeedback(
            `✓ ${item.item_name} (${itemTypeText}) added successfully`,
            'success'
        );
    ...
    function handleItemSelection(item) {
        ...
        if (item.item_type === 'batch' && item.exp_date) {
            const expDate = new Date(item.exp_date);
            if (expDate < new Date()) {
                showFeedback(`✗ Cannot add expired batch: ${item.item_name} (Batch: ${item.batch_no})`, 'error');
                barcodeInput.focus();
                barcodeInput.select();
                return; // STOP: Do NOT add
            }
        ...
        closeItemSelectionModal(); // optional
    }
  ...
  <i class="fas fa-check"></i> Select
        ...
        document.querySelectorAll('.select-item-from-multiple').forEach(btn => {
            btn.addEventListener('click', function () {
                ...
                const barcodeInput = document.getElementById('barcodeInput');
                const stock = parseFloat(selectedItem.current_stock) || 0;

                // CLOSE modal in all cases
                const modalElement = document.getElementById('multipleItemsModal');
                const modalInstance = bootstrap.Modal.getInstance(modalElement);
                if (modalInstance) modalInstance.hide();

                // ❌ Zero stock → feedback only
                if (stock <= 0) {
                    const itemTypeText = selectedItem.item_type === 'batch' ? `(Batch: ${selectedItem.batch_no})` : '';
                    showFeedback(`✗ Out of stock: ${selectedItem.item_name} ${itemTypeText}`, 'error');
                    barcodeInput.focus();
                    barcodeInput.select();
                    return; // Do NOT add
                }

                // ❌ Expired batch → feedback only
                if (selectedItem.item_type === 'batch' && selectedItem.exp_date) {
                    const expDate = new Date(selectedItem.exp_date);
                    if (expDate < new Date()) {
                        showFeedback(`✗ Cannot add expired batch: ${selectedItem.item_name} (Batch: ${selectedItem.batch_no})`, 'error');
                        barcodeInput.focus();
                        barcodeInput.select();
                        return; // Do NOT add
                    }
                ...
                const itemTypeText = selectedItem.item_type === 'batch' ? `batch item ${selectedItem.batch_no}` : 'item';
                showFeedback(`✓ ${selectedItem.item_name} (${itemTypeText}) added successfully`, 'success');
...
<?php echo $__env->make('layouts.footer', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\Sukoyo_Live\resources\views/pos/bill.blade.php ENDPATH**/ ?>