    ...
    <div class="alert alert-success alert-dismissible fade show shadow-sm border-0" role="alert" style="border-left: 4px solid #28a745 !important; border-radius: 8px; background: linear-gradient(135deg, #d4edda 0%, #c3e6cb 100%);">
        <div class="d-flex align-items-start">
            ...
            <div class="flex-grow-1">
                ...
                <p class="mb-0" style="color: #155724; font-size: 14px;">{{ session('success') }}</p>
            </div>
    ...
    <div class="container-fluid mt-3 listtable">
        ...
        <div class="table-wrapper">
            <table class="example table table-bordered">
                ...
                <tbody>
                    @foreach($repackings as $key => $repack)
                    <tr>
                        <td>{{ $loop->iteration }}</td>
                        <td>{{ $repack->item->item_name ?? $repack->item_name ?? '-' }}</td>
                        <td>{{ $repack->total_bulk_qty }} {{ $repack->bulk_unit }}</td>
                        <td>{{ $repack->repack_qty }} {{ $repack->repack_uom }}</td>
                        <td>₹ {{ $repack->cost_per_pack }}</td>
                        <td>₹ {{ $repack->repacking_charge ?? 0 }}</td>
                        <td>₹ {{ $repack->selling_price }}</td>
                        <td>{{ $repack->store->store_name ?? '' }}</td>
                        <td>
                            <div class="d-flex align-items-center gap-2">
                                <a href="#" class="pop-up" data-bs-toggle="modal" data-bs-target="#editRepacking" id="{{ $repack->id }}">
    ...
    <div class="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('inventory.repackingupdate') }}" method="POST">
                    @csrf
                    @method('PUT')
                        ...
                        <div class="p-3" style="background-color: white; border: 1px solid #dee2e6; border-radius: 0.375rem;">
                            <div class="row">
                                <div class="col-md-6 mb-2">
                                    ...
                                    <select class="form-select edit-item-select-{{ $repack->id }}" name="item_id" required>
                                        <option value="{{ $repack->item_id }}" selected>{{ $repack->item->item_name ?? $repack->item_name }}</option>
                                        @foreach ($items as $item)
                                            @if($item->id != $repack->item_id)
                                ...
                                <div class="col-md-6 mb-2">
                                    ...
                                    <div class="inpselectflex">
                                        <input type="number" step="any" class="form-control border-0" name="total_bulk_qty" value="{{ $repack->total_bulk_qty }}" required>
                                        <select class="form-select border-0" name="bulk_unit">
                                            <option value="pcs" {{ $repack->bulk_unit == 'pcs' ? 'selected' : '' }}>pcs</option>
                                            <option value="box" {{ $repack->bulk_unit == 'box' ? 'selected' : '' }}>box</option>
                                            <option value="nos" {{ $repack->bulk_unit == 'nos' ? 'selected' : '' }}>nos</option>
                                            <option value="kg" {{ $repack->bulk_unit == 'kg' ? 'selected' : '' }}>kg</option>
                                            <option value="ltr" {{ $repack->bulk_unit == 'ltr' ? 'selected' : '' }}>ltr</option>
                                        </select>
                                    </div>
                                </div>
                                <div class="col-md-6 mb-2">
                                    <label>Repacking Charge <span class="text-danger">*</span></label>
                                    <input type="number" step="0.01" class="form-control" name="repacking_charge" value="{{ $repack->repacking_charge ?? 0 }}" required>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="p-3" style="background-color: #e9ecef; border-radius: 0.375rem;">
                        ...
                        <div class="row">
                            <div class="col-sm-12 col-md-6 mb-2">
                                ...
                                <select class="form-select edit-variant-select-{{ $repack->id }}" name="variant_name" required>
                                    <option value="{{ $repack->variant_name }}" selected>{{ $repack->variant_name }}</option>
                                </select>
                            </div>
                            <div class="col-sm-12 col-md-6 mb-2">
                                <label>Repack UOM <span class="text-danger">*</span></label>
                                <select class="form-select" name="repack_uom" required>
                                    <option value="kg" {{ $repack->repack_uom == 'kg' ? 'selected' : '' }}>kg</option>
                                    <option value="g" {{ $repack->repack_uom == 'g' ? 'selected' : '' }}>g</option>
                                    <option value="ltr" {{ $repack->repack_uom == 'ltr' ? 'selected' : '' }}>ltr</option>
                                    <option value="litre" {{ $repack->repack_uom == 'litre' ? 'selected' : '' }}>litre</option>
                                    <option value="ml" {{ $repack->repack_uom == 'ml' ? 'selected' : '' }}>ml</option>
                                    <option value="pcs" {{ $repack->repack_uom == 'pcs' ? 'selected' : '' }}>pcs</option>
                                    <option value="pack" {{ $repack->repack_uom == 'pack' ? 'selected' : '' }}>pack</option>
                                    <option value="box" {{ $repack->repack_uom == 'box' ? 'selected' : '' }}>box</option>
                                    <option value="dozen" {{ $repack->repack_uom == 'dozen' ? 'selected' : '' }}>dozen</option>
                                </select>
                            </div>
                            <div class="col-sm-12 col-md-6 mb-2">
                                <label>Repack Qty <span class="text-danger">*</span></label>
                                <input type="number" step="any" class="form-control" name="repack_qty" value="{{ $repack->repack_qty }}" required>
                            </div>
                            <div class="col-sm-12 col-md-6 mb-2">
                                <label>Cost Per Pack <span class="text-danger">*</span></label>
                                <input type="number" step="0.01" class="form-control" name="cost_per_pack" value="{{ $repack->cost_per_pack }}" required>
                            </div>
                            <div class="col-sm-12 col-md-6 mb-2">
                                <label>Selling Price <span class="text-danger">*</span></label>
                                <input type="number" step="0.01" class="form-control" name="selling_price" value="{{ $repack->selling_price }}" required>
                            </div>
                            <div class="col-sm-12 col-md-6 mb-2">
                                <label>Store <span class="text-danger">*</span></label>
                                <select class="form-select" name="store_id" required>
                                    <option value="Warehouse" {{ $repack->store_id == 'Warehouse' ? 'selected' : '' }}>Warehouse</option>
                                    @foreach ($stores as $store)
                                        <option value="{{ $store->id }}" {{ $repack->store_id == $store->id ? 'selected' : '' }}>
                                            {{ $store->store_name }}
                                        </option>
                                    @endforeach
                                </select>
...
<div class="modal fade" id="addRepacking" tabindex="-1" aria-labelledby="addRepackingLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('inventory.repackingstore') }}" method="POST">
                    @csrf
