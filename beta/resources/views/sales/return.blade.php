                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered table-hover" id="vouchersTable">
                            ...
                            <tbody>
                                ...
                                <tr data-status="{{ $voucher->isValid() ? 'valid' : ($voucher->is_used ? 'used' : 'expired') }}">
                                    <td>{{ $index + 1 }}</td>
                                    <td>
                                        <strong class="text-primary">{{ $voucher->voucher_code }}</strong>
                                    </td>
                                    <td>
                                        <span class="text-success fw-bold">₹{{ number_format($voucher->amount, 2) }}</span>
                                    </td>
                                    <td>
                                        <a href="{{ route('sales.profile', ['id' => $voucher->sales_invoice_id]) }}" 
                                           class="text-decoration-none">
                                            BILL-{{ $voucher->sales_invoice_id }}
                                        </a>
                                    </td>
                                    <td>
                                        {{ $voucher->salesInvoice->customer->name ?? 'Walk-in Customer' }}
                                        @if($voucher->salesInvoice->customer)
                                            <br><small class="text-muted">{{ $voucher->salesInvoice->customer->contact }}</small>
                                        @endif
                                    </td>
                                    <td>{{ $voucher->salesInvoice->store->store_name ?? 'N/A' }}</td>
