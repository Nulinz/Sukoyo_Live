...
public function category_bulk_upload(Request $request)
{
    ...
    return redirect()->back()->with('success', 'Categories uploaded successfully!');
}
...
public function subcategory_bulk_upload(Request $request)
{
    ...
    return redirect()->back()->with('success', 'Sub Categories uploaded successfully!');
}
...
public function repacking_store(Request $request)
{
    try {
        ...
        if ($request->item_type === 'new') {
            ...
            $rules['store_id'] = ['required', function($attribute, $value, $fail) {
                if ($value !== 'Warehouse' && !Store::where('id', $value)->exists()) {
                    $fail('The selected store is invalid.');
                }
            }];
...
//     unset($data[0]); // Skip header row

//     foreach ($data as $row) {
//         // CSV Column mapping based on your headers:
//         // 0=item_type, 1=item_code, 2=hsn_code, 3=item_name, 4=brand, 5=category, 
//         // 6=subcategory, 7=discount, 8=sales_price, 9=mrp, 10=wholesale_price, 
//         // 11=measure_unit, 12=opening_stock, 13=opening_unit, 14=gst_rate, 
//         // 15=item_description, 16=stock_status, 17=min_stock, 18=max_stock, 
//         // 19=abc_category, 20=purchase_price, 21=purchase_tax, 22=purchase_gst

//         // Create or get brand (index 4)
//         $brand = Brand::firstOrCreate(
//             ['name' => $row[4] ?? 'Default'], 
//             ['status' => 'Active']
//         );
        
//         // Create or get category (index 5)
//         $category = Category::firstOrCreate(
//             ['name' => $row[5] ?? 'Default'], 
//             ['status' => 'Active']
//         );
        
//         // Create or get subcategory (index 6)
//         $subcategory = SubCategory::firstOrCreate(
//             ['name' => $row[6] ?? 'Default', 'category_id' => $category->id],
//             ['status' => 'Active']
//         );

//         Item::create([
//             'item_type'        => $row[0] ?? 'Product',
//             'item_code'        => $row[1] ?? '',
//             'hsn_code'         => $row[2] ?? '',
//             'item_name'        => $row[3] ?? '',
//             'brand_id'         => $brand->id,
...
//             'discount'         => is_numeric($row[7] ?? 0) ? $row[7] : 0,
//             'sales_price'      => is_numeric($row[8] ?? 0) ? $row[8] : 0,
//             'mrp'              => is_numeric($row[9] ?? 0) ? $row[9] : 0,
//             'wholesale_price'  => is_numeric($row[10] ?? 0) ? $row[10] : 0,
//             'measure_unit'     => $row[11] ?? 'PCS',
//             'opening_stock'    => is_numeric($row[12] ?? 0) ? $row[12] : 0,
//             'opening_unit'     => $row[13] ?? 'PCS',
//             'gst_rate'         => is_numeric($row[14] ?? 0) ? $row[14] : 0,
//             'item_description' => $row[15] ?? '',
//             'stock_status'     => $row[16] ?? 'Active',
//             'min_stock'        => is_numeric($row[17] ?? 0) ? $row[17] : 0,
//             'max_stock'        => is_numeric($row[18] ?? 0) ? $row[18] : 0,
//             'abc_category'     => $row[19] ?? 'C',
//             'purchase_price'   => is_numeric($row[20] ?? 0) ? $row[20] : 0,
//             'purchase_tax' => !empty(trim($row[21])) ? trim($row[21]) : 'With Tax',
//             'purchase_gst'     => is_numeric($row[22] ?? 0) ? $row[22] : 0,
//         ]);
//     }

//     return redirect()->back()->with('success', 'Items uploaded successfully!');
// }
...
public function item_bulk_upload(Request $request)
{
    ...
    foreach ($data as $rowIndex => $row) {
        try {
            ...
            $getValue = function($index, $default = '') use ($row) {
                return isset($row[$index]) && trim($row[$index]) !== '' ? trim($row[$index]) : $default;
            };

            // Validate required fields
            if (empty($getValue(3))) { // item_name is required
            ...
            $categoryName = $getValue(5, 'Default');
            $category = Category::firstOrCreate(
                ['name' => $categoryName], 
                ['status' => 'Active']
            );
            ...
            $subcategoryName = $getValue(6, 'Default');
            $subcategory = SubCategory::firstOrCreate(
            ...
            Item::create([
                ...
                'category_id'      => $category->id,
                'subcategory_id'   => $subcategory->id,
                'discount'         => $numericValue(7, 0),
                'sales_price'      => $numericValue(8, 0),
                'mrp'              => $numericValue(9, 0),
                'wholesale_price'  => $numericValue(10, 0),
                'measure_unit'     => $getValue(11, 'PCS'),
                'opening_stock'    => $numericValue(12, 0),
                'opening_unit'     => $getValue(13, 'PCS'),
                'gst_rate'         => $numericValue(14, 0),
                'item_description' => $getValue(15),
                'stock_status'     => $getValue(16, 'Active'),
                'min_stock'        => $numericValue(17, 0),
                'max_stock'        => $numericValue(18, 0),
                'abc_category'     => $getValue(19, 'C'),
                'purchase_price'   => $numericValue(20, 0),
                'purchase_tax'     => $getValue(21, 'With Tax'),
                'purchase_gst'     => $numericValue(22, 0),
            ]);
            ...
            \Log::error("Failed to import row {$rowIndex}: " . $e->getMessage());
        }
    }
    ...
    return redirect()->back()->with($errorCount > 0 ? 'warning' : 'success', $message);
}
    ...
    try {
        ...
        foreach ($purchaseOrderItems as $itemEntry) {
            ...
            if ($itemEntry->purchaseInvoice) {
                ...
                if ($itemEntry->purchaseInvoice->purchaseOrder && 
